...
// hit the following statement in console
// $x('//*[@id="pagebody"]/table/tbody/tr/th').map((x) => /(\d+), (\d+), (\d+)/.test(x.style.background) ? [x.innerText, RegExp.$1, RegExp.$2, RegExp.$3] : null)
...
// パクリ元: https://qiita.com/akebi_mh/items/3377666c26071a4284ee
function rgb2hsv(r: number, g: number, b: number): {
    ...
    const v = Math.max(r, g, b)
        , d = v - Math.min(r, g, b)
        , s = v ? d / v : 0
        , h = s ? (v === r ? (g - b) / d + (g < b ? 6 : 0) : v === g ? 2 + (b - r) / d : 4 + (r - g) / d) * 60 : 0;
